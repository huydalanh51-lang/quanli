...
.webgis-mobile-ai-btn:hover {
  transform: translateY(-1px);
  box-shadow: 0 20px 42px rgba(194, 65, 12, 0.28);
}
.webgis-mobile-ai-icon {
  font-size: 23px;
  line-height: 1;
}
.webgis-mobile-ai-btn span:last-child {
  font-size: 11px;
  line-height: 1;
}
@media (max-width: 1180px) {
...
@media (max-width: 760px) {
  ...
  body.webgis-mode .webgis-mobile-ai-btn {
    display: flex;
  }
  body.ai-webgis-assistant .webgis-mobile-ai-btn {
    display: none;
  }
  body.webgis-mode.ai-webgis-assistant .ai-panel {
    inset: auto 10px 10px 10px;
    z-index: 7000;
    width: auto;
    max-height: min(70dvh, 470px);
  }
  body.webgis-mode.ai-webgis-assistant .ai-card {
    max-height: min(70dvh, 470px);
  }
  body.webgis-mode.ai-webgis-assistant .ai-messages {
    max-height: calc(70dvh - 230px);
  }
}
"""
...
<main id="webgisPage" class="webgis-page" aria-label="WebGis">
  ...
  <button id="webgisMobileAiBtn" class="webgis-mobile-ai-btn" type="button" aria-label="M&#7903; tr&#7907; l&#253; AI WebGIS" title="Tr&#7907; l&#253; AI">
    <span class="webgis-mobile-ai-icon" aria-hidden="true">&#129302;</span>
    <span>AI</span>
  </button>
</main>
"""
...
function webgisRequireAdmin(message = 'Vui lòng đăng nhập để tiếp tục.') {
...
async function webgisLoginAdmin() {
  ...
  if (!username || !password) {
    webgisSetAdminStatus('Vui lòng nhập tài khoản và mật khẩu.', true);
    return;
  }
  ...
  if (!response.ok) {
    webgisSetAdminStatus(payload.error || 'Không đăng nhập được.', true);
    return;
  }
  ...
  webgisSetAdminStatus('Đã đăng nhập thành công.');
  webgisSetSaveStatus('Đã đăng nhập');
  webgisInvalidateSize(80);
}
...
async function webgisEnsureLayerLoaded(layerId, options = {}) {
  const normalizedLayerId = String(layerId || '');
...
async function webgisSaveLayerFeatures(layerId) {
  ...
  if (response.status === 401) {
    ...
    webgisSetSaveStatus('Phiên đăng nhập đã hết hạn. Đăng nhập lại để lưu layer.', true);
    return null;
  }
...
async function webgisSaveNow() {
  ...
  try {
    ...
    if (response.status === 401) {
      webgisClearAdminSession();
      webgisSetSaveStatus('Phiên đăng nhập đã hết hạn. Bấm Đăng nhập khi cần quản trị.', true);
      return;
    }
    if (!response.ok) throw new Error(await response.text());
...
async function webgisPatchLayerMetadata(layerId) {
  ...
  if (response.status === 401) {
    webgisClearAdminSession();
    webgisShowAdminLogin('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.');
  }
...
function webgisBindEvents() {
  ...
  webgisEl('webgisMobileAiBtn')?.addEventListener('click', () => openAiAssistant('webgis'));
  ...
  webgisEl('webgisAdminLogoutBtn').addEventListener('click', () => {
    webgisClearAdminSession();
    webgisSetSaveStatus('Đã đăng xuất');
    webgisShowAdminLogin('Đã đăng xuất.');
  });
...
async function initializeWebGIS(options = {}) {
  ...
  webgisState.initializing = (async () => {
    try {
      ...
      const layersToLoad = webgisState.layerDefs.filter(def =>
        def.is_public !== false &&
        (def.visible === true || def.default_visible === true || Number(def.feature_count || 0) > 0)
      );
      if (layersToLoad.length) {
        StartupScreen?.setStatus?.(`Đang nạp ${layersToLoad.length} lớp dữ liệu WebGIS...`);
        await Promise.all(layersToLoad.map(def => webgisEnsureLayerLoaded(def.id, { strict: options.requireLayerData === true })));
      }
      webgisRebuildOverlays();
      webgisBindEvents();
      webgisUpdateAdminUi();
...
body.module-mode #sideUtilitiesBtn,
body.docs-mode #sideUtilitiesBtn,
...
body.webgis-mode #sideUtilitiesBtn,
body.webgis-mode #sideWebGisBtn {{
  display: none;
}}
.side-utility-menu,
...
.side-utility-menu[hidden],
.home-utility-menu[hidden] {{
  display: none;
}}
.side-utility-menu {{
  position: fixed;
  ...
  border-radius: 14px;
  background: rgba(255,255,255,0.97);
  box-shadow: 0 18px 38px rgba(15, 47, 87, 0.16);
  backdrop-filter: blur(12px);
}}
.side-utility-menu button,
...
.side-utility-menu button:hover,
.home-utility-menu button:hover {{
  border-color: #93c5fd;
  background: #eff6ff;
  color: #1d4ed8;
}}
...
<aside class="app-sidebar" aria-label="Điều hướng module">
  ...
  <div id="sideUtilitiesMenu" class="side-utility-menu" hidden>
    <button id="sideUtilityLandBtn" type="button">Chu chuyển đất đai</button>
    <button id="sideUtilityLibraryBtn" type="button">Thư viện PDF</button>
  </div>
</aside>
  ...
  <section class="dashboard">
    ...
    <div class="module-grid" aria-label="Danh sách module">
      <article class="module-card">
        ...
        <span class="module-card-tag">Công cụ nghiệp vụ</span>
        <h2>Tiện ích</h2>
        <p>Truy cập nhanh biểu chu chuyển đất đai và thư viện tài liệu PDF trong cùng một nhóm tiện ích gọn gàng.</p>
        <button id="homeUtilitiesBtn" class="primary" type="button">Mở tiện ích</button>
        <div id="homeUtilitiesMenu" class="home-utility-menu" hidden>
          <button id="homeLandTransferBtn" type="button">Chu chuyển đất đai</button>
          <button id="homeLibraryBtn" type="button">Thư viện PDF</button>
        </div>
      </article>
      <article class="module-card">
        <div class="module-card-icon" aria-hidden="true">◎</div>
...
const StartupScreen = (() => {{
  ...
  function show(text = 'Đang kiểm tra máy chủ') {{
    if (!screen) return;
    clearInterval(timer);
    screen.hidden = false;
  ...
  async function waitForBackend() {{
    if (!canCheckBackend) return true;
    while (true) {{
      attempts += 1;
      try {{
        const response = await fetch('/health', {{
        ...
        if (response.status === 200) {{
          setStatus('Máy chủ đã sẵn sàng, đang nạp dữ liệu WebGIS...');
          return true;
        }}
        setStatus(`Máy chủ đang khởi động (${{response.status}})`);
      }} catch (error) {{
  ...
  return {{ start, show, hide, checkHealth, waitForBackend, setStatus }};
}})();
StartupScreen.start();
...
function closeUtilityMenus() {{
  const sideMenu = $('#sideUtilitiesMenu');
  const homeMenu = $('#homeUtilitiesMenu');
  if (sideMenu) sideMenu.hidden = true;
  if (homeMenu) homeMenu.hidden = true;
}}
...
async function showWebGisPage() {{
  closeUtilityMenus();
  closeMainMenu();
  StartupScreen.show('Đang chuẩn bị WebGIS và nạp dữ liệu layer...');
  try {{
    await StartupScreen.waitForBackend();
    ...
    $('#importLog').hidden = true;
    webgisUpdateAdminUi();
    await initializeWebGIS({{ requireLayerData: true }});
    StartupScreen.hide();
  }} catch (error) {{
    webgisSetSaveStatus('Không khởi động được WebGIS', true);
    StartupScreen.setStatus('Chưa nạp được dữ liệu WebGIS. Hệ thống sẽ thử lại khi bạn mở lại WebGIS.');
    document.body.classList.add('home-mode');
    document.body.classList.remove('webgis-mode');
    window.setTimeout(() => StartupScreen.hide(), 900);
    alert(error.message || String(error));
  }}
}}

$('#sideHomeBtn').addEventListener('click', showHomePage);
$('#sideUtilitiesBtn').addEventListener('click', event => {{
  event.stopPropagation();
  const menu = $('#sideUtilitiesMenu');
  menu.hidden = !menu.hidden;
}});
$('#sideUtilityLandBtn').addEventListener('click', showLandTransferPage);
$('#sideUtilityLibraryBtn').addEventListener('click', showDocumentLibraryPage);
$('#sideWebGisBtn').addEventListener('click', showWebGisPage);
$('#homeUtilitiesBtn').addEventListener('click', () => {{
  const menu = $('#homeUtilitiesMenu');
  menu.hidden = !menu.hidden;
}});
...
$$('.side-utility-menu, .home-utility-menu').forEach(panel => {{
  panel.addEventListener('click', event => event.stopPropagation());
}});
document.addEventListener('click', event => {{
  if (!event.target.closest('.main-menu')) closeMainMenu();
  closeToolDropdowns();
  if (!event.target.closest('#sideUtilitiesBtn') && !event.target.closest('#homeUtilitiesBtn')) closeUtilityMenus();
}});
document.addEventListener('keydown', event => {{
  if (event.key === 'Escape') {{
    closeMainMenu();
...
function openAiAssistant(mode = 'land-transfer') {{
  ...
  refreshAiStatus();
  const shouldFocusQuestion = !(isWebgis && window.matchMedia?.('(max-width: 760px)').matches);
  if (shouldFocusQuestion) $('#aiQuestion').focus();
}}
